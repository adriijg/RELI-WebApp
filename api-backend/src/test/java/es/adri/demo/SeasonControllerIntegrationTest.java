...
class SeasonControllerIntegrationTest {
    ...
    @Test
    void getSeasonsIsPublic() throws Exception {
        seasonRepository.save(new Season(null, "2025/2026", true));
    ...
    @Test
    void createSeasonRequiresAdminRole() throws Exception {
        User user = createUser("user1", "user1@example.com", "Password123", Role.ROLE_USER);
        SeasonRequestDTO request = new SeasonRequestDTO("2025/2026", true);
    ...
    @Test
    void createSeasonAsAdminReturnsCreated() throws Exception {
    ...
    @Test
    void updateSeasonAsAdminReturnsUpdatedSeason() throws Exception {
        User admin = createUser("admin", "admin@example.com", "Password123", Role.ROLE_ADMIN);
        Season season = seasonRepository.save(new Season(null, "2025/2026", true));
        SeasonRequestDTO request = new SeasonRequestDTO("2026/2027", false);

        mockMvc.perform(put("/api/seasons/{id}", season.getId())
                        .header("Authorization", basicAuth(admin.getUsername(), "Password123"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
    ...
    @Test
    void deleteSeasonAsAdminReturnsNoContent() throws Exception {
        User admin = createUser("admin", "admin@example.com", "Password123", Role.ROLE_ADMIN);
        Season season = seasonRepository.save(new Season(null, "2025/2026", true));

        mockMvc.perform(delete("/api/seasons/{id}", season.getId())
                        .header("Authorization", basicAuth(admin.getUsername(), "Password123")))
